describe('Filter layers', () => {
  it('passes', () => {
    ...
    cy.get('span:contains("ÖREB-Kataster") > button[data-test-id="add-active-map"]',{timeout:5000}).should('be.visible');
    ...
    cy.get('span:contains("Verkehrstechnik (BSA)") > button[data-test-id="add-active-map"]',{timeout:5000}).should('be.visible');
  });
});
